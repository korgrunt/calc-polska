...
public class Server {
    ...
    private static String manageRequest(Socket socket, String requestStr) {
        String result;
        List<String> request;

        request = Arrays.asList(requestStr.split("\r"));

        if (request.get(0).contains("EXIT") || request.get(0).contains("QUIT")) {
            result = "EXIT";
        }
                    ...
                    while ((c = reader.read()) != -1) {
                        ...
                        if (c == '\n') {
                            ...
                            if (requestStr.endsWith("\r\n")) {
                                String instruction = manageRequest(socket, requestStr);
                                this.alive = instruction.equals("CONTINUE");
                            }
                        }
                    }
                    socket.close();
